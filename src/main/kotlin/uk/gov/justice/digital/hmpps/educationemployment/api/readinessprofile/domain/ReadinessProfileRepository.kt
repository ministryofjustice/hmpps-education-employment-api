...
interface ReadinessProfileRepository :
  ...
  @Query(
    ...
    WITH recent_updates AS (
      ...
      SELECT p.offender_id, 
        p.profile_data->>'prisonId' AS prison_id,
        l.within_12_weeks,
        p.profile_data->>'status' AS work_status
      FROM profile_snapshots p INNER JOIN latest l
      ON L.offender_id = p.offender_id AND l.rev_number = p.rev_number
      WHERE p.profile_data->>'prisonId' = :prisonId
    )
    SELECT work_status as field,
      COUNT(CASE within_12_weeks WHEN TRUE THEN 1 ELSE NULL END) AS countWithin12Weeks,
      COUNT(CASE within_12_weeks WHEN FALSE THEN 1 ELSE NULL END) AS countOver12Weeks
  ...
  fun countWorkStatusByPrisonIdAndDateTimeBetween(
    prisonId: String,
    startTime: Instant,
    endTime: Instant,
  ): List<MetricsCountByStringField>
  ...
  @Query(
    ...
    WITH recent_updates AS (
      ...
      SELECT offender_id, MAX(rev_number) AS rev_number, 
      	BOOL_OR(COALESCE((profile_data->>'within12Weeks')::boolean, TRUE)) as within_12_weeks, 
      	BOOL_OR(COALESCE((profile_data->>'statusChange')::boolean, FALSE)) AS statusChange
      FROM profile_snapshots GROUP BY offender_id
    ), latest_profiles AS (
      SELECT p.offender_id, 
        p.profile_data->>'prisonId' AS prison_id,
        l.within_12_weeks, l.statusChange
      FROM profile_snapshots p INNER JOIN latest l
      ON L.offender_id = p.offender_id AND l.rev_number = p.rev_number
      WHERE p.profile_data->>'prisonId' = :prisonId
    )
    SELECT COUNT(1) FROM latest_profiles WHERE statusChange = TRUE;
    """,
    nativeQuery = true,
  )
  fun countWorkStatusChangeByPrisonIdAndDateTimeBetween(
    prisonId: String,
    startTime: Instant,
    endTime: Instant,
  ): Long
  ...
  @Query(
    ...
    SELECT 
      COUNT(CASE within_12_weeks WHEN TRUE THEN 1 ELSE NULL END) AS countWithin12Weeks,
      COUNT(CASE within_12_weeks WHEN FALSE THEN 1 ELSE NULL END) AS countOver12Weeks,
      COUNT(CASE work_status WHEN 'SUPPORT_DECLINED' THEN 1 ELSE NULL END) as countSupportDeclined,
      COUNT(CASE work_status WHEN 'NO_RIGHT_TO_WORK' THEN 1 ELSE NULL END) as countNoRightToWork
    FROM latest_profiles;
    """,
    nativeQuery = true,
  )
  fun countSummaryByPrisonIdAndDateTimeBetween(
    prisonId: String,
    startTime: Instant,
    endTime: Instant,
  ): MetricsSummaryCount
