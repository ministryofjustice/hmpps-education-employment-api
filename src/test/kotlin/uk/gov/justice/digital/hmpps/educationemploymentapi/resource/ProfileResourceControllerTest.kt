...
class ProfileResourceControllerTest {
  ...
  fun `Test Post  of a add profile notes`() {
    ...
    val result = mvc.perform(post("/readiness-profiles/A1234AB/notes/$disclosureLetter").accept(APPLICATION_JSON).content(TestData.noteFreeTextJson).contentType(APPLICATION_JSON).param("oauth2User", "ssss").headers((setAuthorisation(roles = listOf("WORK_READINESS_EDIT")))))
      .andExpect(status().isOk)
      .andExpect(content().contentType(APPLICATION_JSON))
      .andReturn()
    var retreivedNotesList: MutableList<Note> = mapper.readValue(result.response.contentAsString, object : TypeReference<MutableList<Note>>() {})
  ...
  fun `Test Post  of a add profile `() {
    ...
    val result = mvc.perform(post("/readiness-profiles/A1234AB").accept(APPLICATION_JSON).content(TestData.createProfileJsonRequest).contentType(APPLICATION_JSON).param("oauth2User", "ssss").headers((setAuthorisation(roles = listOf("WORK_READINESS_EDIT")))))
      .andExpect(status().isOk)
      .andExpect(content().contentType(APPLICATION_JSON))
      .andReturn()
  ...
  fun `Test Get  of an  profile list for offenders `() {
    ...
    val result = mvc.perform(post("/readiness-profiles/search").accept(APPLICATION_JSON).content(TestData.offenderIdListjson).contentType(APPLICATION_JSON).param("oauth2User", "ssss").headers((setAuthorisation(roles = listOf("WORK_READINESS_EDIT")))))
      .andExpect(status().isOk)
      .andExpect(content().contentType(APPLICATION_JSON))
      .andReturn()
